...
const Liabilities = () => {
    ...
    const getOperationsLiabilities = useCallback(() => {
        PeatioApiService.getOperationsLiabilities(page, limit, filters).then((res) => {
    ...
    useEffect(() => {
        getOperationsLiabilities(limit, page);
    }, [getOperationsLiabilities, page]);
    ...
    return <>
        <Row justify="end" style={{ marginBottom: 16 }}>
            ...
            <Button
                ...
                size="middle"
                onClick={() => {
                    getOperationsLiabilities(page, limit);
                }}
            >REFRESH</Button>
            </Space>
        </Row>
        <Table rowKey={'index'} dataSource={liabilities} columns={columns} onChange={onChange} pagination={{ pageSize: limit, current: page, total: totalRows }}/>
