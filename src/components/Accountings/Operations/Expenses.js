...
const Expenses = () => {
    ...
    const getOperationsExpenses = useCallback(() => {
        PeatioApiService.getOperationsExpenses(page, limit, filters).then((res) => {
    ...
    useEffect(() => {
        getOperationsExpenses(limit, page);
    }, [getOperationsExpenses, page]);
    ...
    return <>
        <Row justify="end" style={{ marginBottom: 16 }}>
            ...
            <Button
                ...
                size="middle"
                onClick={() => {
                    getOperationsExpenses(page, limit);
                }}
            >REFRESH</Button>
            </Space>
        </Row>
        <Table rowKey={'index'} dataSource={expenses} columns={columns} onChange={onChange} pagination={{ pageSize: limit, current: page, total: totalRows }}/>
