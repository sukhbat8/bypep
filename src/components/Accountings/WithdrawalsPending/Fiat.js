...
const Fiat = () => {
    ...
    const getWithrawalFiat = useCallback(() => {
        PeatioApiService.getCurrencys().then((res) => {
            const bene = res.find(x => x.type === 'fiat');
            PeatioApiService.getWithrawalFiat(page, limit, filters, bene.type, state).then((res) => {
    ...
    useEffect(() => {
        getWithrawalFiat(page, limit);
    }, [getWithrawalFiat, page]);
